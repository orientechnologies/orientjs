...
describe("ODatabase API - Transaction", function() {
  ...
  describe("ODatabase::commit()", function() {
    ...
    it("should perform a single action", function() {
      ...
      return this.tx.commit().then(function(results) {
  ...
  describe("ODatabase::Transaction SQL", function() {
    ...
    it("should create a record in tx with sql", function() {
      ...
      return this.db
        .command("insert into TestClassTx set name = 'Foo'")
        .all()
        .bind(this)
        .then(function(res) {
  ...
  describe("ODatabase::Transaction Complex", function() {
    ...
    it("should create a record in tx consistent with all apis", function() {
      this.db.begin();
      return this.db.class
        ...
        .then(record => {
          record["@rid"].should.be.instanceOf(RID);
          RID.isTemporary(record["@rid"]).should.be.eql(true, record["@rid"]);
          return this.db.query("select * from TestClassTX").all();
        })
        .then(res => {
          res.length.should.equal(1);
    ...
    it("should delete a record in tx consistent with all apis", function() {
      this.db.begin();
      return this.db.class
        .get("TestClassTx")
        .then(cls => {
        ...
        .then(results => {
          ...
          results.deleted.length.should.equal(0);
          this.db.begin();
          return this.db.record.delete(results.created[0]);
        })
        .then(res => {
          return this.db.tx().commit();
        })
    ...
    it("should update a record in tx consistent with all apis", function() {
      this.db.begin();
      return this.db.class
        .get("TestClassTx")
        .then(cls => {
        ...
        .then(results => {
          ...
          results.deleted.length.should.equal(0);
          this.db.begin();
          results.created[0].name = "item5";
          return this.db.record.update(results.created[0]);
        })
        .then(res => {
          return this.db.tx().commit();
        })
        ...
        .then(res => {
          res[0].name.should.be.eql("item5");
          res.length.should.equal(1);
        });
    });
  });
