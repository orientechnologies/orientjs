...
describe("ODatabaseSession API - Open / Simple Query", function () {
  ...
  describe("Database::Links & Embedded", function () {
    ...
    it("should create a simple record with link map", function () {
      ...
      return this.db.record
        ...
        .then((response) => {
          return this.db
            .command(
              `insert into Foo set linkMap = { "foo" : ${response["@rid"]}, "bar" : null }`
            )
            .one();
        })
        .then((response) => {
    ...
    it("should create a complex record with embeded map", function () {
      return this.db
        .command(
          `insert into Foo set embeddedMap = { "foo" :  { "nested" : true}, "bar" : null }`
        )
        .one()
        .then((response) => {
          response.embeddedMap.foo.should.eql({ nested: true });
          should(response.embeddedMap["bar"]).be.null;
        });
    });

    it("should create a complex record with embeded map and read the projection", function () {
      return this.db
        .command(
          `insert into Foo set embeddedMap = { "foo" :  { "nested" : true }, arr : [32, { "name" : "test"}], "bar" : null }`
        )
        .one()
        .then((response) => {
          return this.db
            .query("select embeddedMap from " + response["@rid"])
            .one();
        })
        .then((response) => {
          response.embeddedMap.foo.nested.should.eql(true);
          should(response.embeddedMap["bar"]).be.null;
        });
    });
  ...
  describe("Database::RidBags", function () {
    before(function () {
      return TEST_CLIENT.session({ name: "test_complex_records" }).then(
        (db) => {
          this.db = db;
        }
      );
