"use strict";
var Index = require("../../lib/client/database/idx/index"),
...
describe("Database API - Index", function() {
  before(CAN_RUN(37,function () {
    return CREATE_TEST_DB(this, "testdb_dbapi_index")
      .bind(this)
      .then(() => {
        return TEST_CLIENT.session({ name: "testdb_dbapi_index" });
      })
      .then(db => {
        this.db = db;
      })
  ...
  describe("Db::index.get()", function() {
    it("should get an index by name", function() {
      return this.db.index.get("TestClass.name").then(function(index) {
        index.should.be.an.instanceOf(Index);
      });
    });
  });
  ...
  describe("Db::index::*", function() {
    ...
    describe("Db::index::add()", function() {
      ...
      it("should add multiple records into the index", function() {
        return this.index
          .add(
            this.items.slice(1).map(function(item) {
              return {
                key: item.name,
                rid: item["@rid"]
              };
