...
RawExpression.prototype.toString = function () {
  return this.alias ? (this.value + " AS " + this.alias) : this.value;
};

Statement.RawExpression = RawExpression;
 ...
 * @return {Statement}                     The statement object.
 */
Statement.prototype.from = function (name) {
  this._state.from = this._state.from || [];
  this._state.from.push(name);
  return this;
};
...
Statement.prototype.increment = function (property, value) {
  ...
  this._state.increment.push([property, value === undefined ? 1 : value]);
  return this;
};
...
Statement.prototype.like = whereClause('and', 'LIKE');
Statement.prototype.gt = whereClause('and', '>');
Statement.prototype.gte = whereClause('and', '>=');
Statement.prototype.lt = whereClause('and', '<');
...
/**
 * Add the maxdepth clause to a traverse statement.
 *
 * @param  {Integer} args The maximum depth of the traverse statement.
 * @return {Statement}            The statement object.
 */
Statement.prototype.maxdepth = clause('maxdepth');

/**
 * Unwind with one or more columns.
 *
 * @param  {String|String[]} args The columns to unwind.
 * @return {Statement}            The statement object.
 */
Statement.prototype.unwind = clause('unwind');
 ...
 * > use `OrientDB.utils.encode()` if you need to allow safe values here.
 *
...
Statement.prototype.let = function (name, value) {
  ...
  this._state.bcommon = this._state.bcommon || [];
  this._state.bcommon.push([name, value]);
  return this;
};
 ...
 * > use `OrientDB.utils.encode()` if you need to allow safe values here.
 *
 * @param  {String}           condition  The condition of the variable to assign
 * @param  {String|Statement} statements The statements of the variable, can be an SQL statement.
 * @return {Statement}              The statement object.
 */
Statement.prototype.if = function (condition, statements) {
  /* this._state['if'] = this._state['if'] || [];
   this._state['if'].push([condition, statements]);*/
  this._state.bcommon = this._state.bcommon || [];
  this._state.bcommon.push([condition, statements]);
  return this;
};
 ...
 * sleep a transaction.
 *
 * @param  {Integer} retryLimit The sleep time in miliseconds, defaults to 0.
 * @return {Statement}          The statement object.
 */
Statement.prototype.sleep = function (ms) {
  this._state.sleep = ms || 0;
  return this;
};

/**
 * Specifiy a rollback clause for the statement.
 *
 * @param  {String|Object} args   The rollback clause.
 * @return {Statement}            The statement object.
 */
Statement.prototype.rollback = clause('rollback');
...
Statement.prototype.buildStatement = function () {
  ...
  if (state.commit !== undefined) {
    ...
    /*if (state.if && state.if.length) {
      statement.push(state.if.map(function (item) {
        if (Array.isArray(item[1]) === true) {
          var sts = item[1].map(function (stItem) {
            //var innerStm = new Statement(self.db);
            stItem._state.paramIndex = self._state.paramIndex;
  ...
  if (state.maxdepth >= 0) {
    statement.push('MAXDEPTH ' + state.maxdepth);
  }
