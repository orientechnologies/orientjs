 ...
 * > NOTE: You must have installed the lucene query plugin in OrientDB for this to work.
 *
 * @param  {String|Object} longitudeProperty Either the name of the longitude property,
 *                                           or a map of field names to values.
 * @param  {String|Number} latitudeProperty  Either the name of the latitude property,
 ...
 * @return {Statement}                       The statement object.
 */
Statement.prototype.near = function (longitudeProperty, latitudeProperty, longitude, latitude, maxDistanceInKms) {
  var properties = [],
      values = [];
  if (typeof longitudeProperty === 'object') {
    var keys = Object.keys(longitudeProperty),
        length = keys.length,
        key, i;
    for (i = 0; i < length; i++) {
      key = keys[i];
      properties.push(key);
      values.push(longitudeProperty[key]);
    }
    maxDistanceInKms = latitudeProperty;
  }
  else {
    properties.push(longitudeProperty, latitudeProperty);
 ...
 * @return {Statement}                       The statement object.
 */
Statement.prototype.within = function (longitudeProperty, latitudeProperty, box) {
  return this.where(
    '['+longitudeProperty+','+latitudeProperty+'] WITHIN '+JSON.stringify(box)
  );
};
