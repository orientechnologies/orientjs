...
Class.prototype.configure = function (config) {
  ...
  this.overSize = config.overSize || 0;
  ...
  this.superClass = config.superClasses ? config.superClasses.join(', ') : config.superClass || null;
  ...
  return this.send('record-load', {
    ...
    .then(function (response) {
      ...
      if (!record || !record.classes) {
        ...
        //console.log('record', record.classes);
...
exports.update = function (cls, reload) {
  ...
  for (var attr in attributes) {
    if (cls[attr] !== undefined) {
      if (attr === 'superClasses') {
        cls[attr] = cls[attr].join(', ');
      }

      promises.push(this.exec(prefix + attributes[attr] + ' ' + cls[attr]));
    }
  }
  ...
  return Promise.all(promises)
    .bind(this)
    .then(function () {
      return this.class.get(cls.changeName || cls.name, reload);
    });
};
