...
var utils = require('../utils'),
  ...
  SqlFunction = require('./sqlfunction'),
...
function Db(config) {
  ...
  this.augment('sequence', require('./sequence'));
}
...
LiveQuery.prototype.handleLiveQuery = function (currentToken, operation, result) {
  ...
  if (currentToken === this.config.token) {
    if (operation === 1) {
      this.emit("live-update", result);
    } else if (operation === 2) {
      this.emit("live-delete", result);
    } else if (operation === 3) {
      this.emit("live-insert", result);
      this.emit("live-create", result);
    }
  }
};

LiveQuery.prototype.handleLiveQueryEnd = function(currentToken) {
  if (currentToken === this.config.token) {
    this.emit('live-end');
  }
};

LiveQuery.prototype.unsubscribe = function() {

    if(!this.config.token){
      return Promise.reject("Cannot unsubscribe the live query. Token is missing");
    }else {
      if(!this.unsubscribing){
        this.unsubscribing = 
          this.config.session.query("live unsubscribe :token",
...
Db.prototype.open = function () {
  ...
  return this.server.send('db-open', {
    ...
    .then(function (response) {
      ...
      if (response.serverCluster && this.server.transport.pool) {
        this.server.transport.pool.emit('update-config', response.serverCluster);
      }
  ...
  this.exec(command, options)
    ...
    .then(function (response) {
      if (response.length > 0) {
        ...
        this.server.transport.connection.on('live-query-end',function(token){
          live.handleLiveQueryEnd(token);
        });
...
/**
 * Create a call for sql Function.
 *
 * @return {SqlFunction} The SqlFunction instance.
 */
Db.prototype.sqlFunction = function () {
 ...
 * @return {Query} The query instance.
 */
Db.prototype.if = function () {
  var query = this.createQuery();
  return query.if.apply(query, arguments);
};
