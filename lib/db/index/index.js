...
function Index (config) {
  config = config || {};
  if (!(this instanceof Index)) return new Index(config);
...
Index.prototype.add = function (args) {
  ...
  return Promise.map(args, function (item) {
    return this.db.query('INSERT INTO index:' + this.name + ' (key, rid) VALUES ("' + this.db.escape(item.key) + '", ' + this.db.escape(item.rid) + ')');
  }.bind(this));
};
...
Index.prototype.set = function (key, value) {
  return this.db.query('INSERT INTO index:' + this.name + ' (key, rid) VALUES ("' + this.db.escape(key) + '", ' + this.db.escape(value['@rid'] || value) + ')')
  .return(this);
};
