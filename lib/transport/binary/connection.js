...
Connection.prototype._sendOp = function (op, params) {
  ...
  return new Promise(function (resolve, reject) {
    if (typeof op === 'string' && self.protocol) {
    ...
    var command = [op, {resolve: resolve, reject: reject}];
    self.queue.push(command);

    if(self.timeout) {
      setTimeout(function(){
        if(command[1].completed !== true) {
          if(!self.timeoutTriggered){
            self.timeoutTriggered = true;
            var msg = 'Operation ' + op.id  +  ' went in timeout on server ' + self.host + ":" + self.port;
            self.logger.debug(msg);
            self.handleOperationTimeout(msg);
          }
        }
      },self.timeout);
    }
...
Connection.prototype.negotiateProtocol = function () {
  return new Promise(function (resolve, reject) {
    ...
    this.socket.once('end', function (err) {
      this.logger.debug('connection ended during protocol negotiation: ' + err);
      if (this.socket) {
        this.socket.removeAllListeners(); // close earlier could have destroyed & set socket to null.
      }
      this.connecting = false;
...
/**
 * Handle a Operation timeout event.
 *
 * @param  {Error} err The error object, if any.
 */
Connection.prototype.handleOperationTimeout = function (err) {
  this.destroySocket();
  this.cancel(new errors.Connection(1, err));
};
...
Connection.prototype.process = function (buffer, offset) {
  ...
  if (this.queue.length === 0) {
    ...
    if (status === OperationStatus.PUSH_DATA) {
      ...
      return [offset, status];
    }else if (status === OperationStatus.LIVE_RESULT_END) {
      token = parsed[1];
      offset = parsed[2];
      this.emit('live-query-end', token, operation);
      return [offset, status];
    }
  }
  ...
  while ((item = this.queue.shift())) {
    ...
    if (status === OperationStatus.READING) {
      ...
      token = parsed[1];
      offset = parsed[2];
      this.emit('live-query-end', token);
      this.queue.unshift(item);
      //return offset;
    }
