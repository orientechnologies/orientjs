...
BinaryTransport.prototype.autoReconnect = function () {
...
BinaryTransport.prototype.send = function (operation, options) {
  ...
  if (~this.sessionId || options.sessionId != null) {
    ...
    var self = this;
    return new Promise(function (resolve, reject) {
      (self.pool || self.connection).send(operation, options).then(resolve).catch(function (err) {
        // If connection error try to reconnect and resend the request
        if (err.code == 1 || err.code == 2) {
          self.autoReconnect().then(function (autoConnect) {
            if (options.db) {
