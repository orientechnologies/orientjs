...
BinaryTransport.prototype.configure = function (config) {
  ...
  this.distributedConfigPush= (config.distributedConfigPush!=null) ? config.distributedConfigPush : true;
...
BinaryTransport.prototype.autoReconnect = function (err) {
  if (this.autoReconnection) {
    ...
    if (this.autoReconnectRetries++ < this.maxRetries && this.shiftServer()) {
      reconnectTransport(this);
      var self = this;
      this.autoReconnection = new Promise(function (resolve, reject) {
        self.connect().then(function (response) {
          self.autoReconnection = null;
          resolve(response);
        }).catch(reject);
      });
      return this.autoReconnection;
    } else {
      this.autoReconnection = null;
      this.autoReconnectRetries = 0;
...
function reconnectTransport(transport, cancellationError) {
  ...
  if(!transport.pool){
    ...
    transport.pool.cancel(cancellationError);
    transport.connection = false;
    transport.configurePool(transport.poolCfg);
    transport.emit('reset');
  }
}
