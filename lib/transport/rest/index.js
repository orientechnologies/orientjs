...
RestTransport.prototype.process = function (op, params) {
  ...
  return axios(prepared)
    .then(function (response) {
      return self.handleResponse(op, prepared, response);
    })
    .then(op.processResponse.bind(op))
    .catch(function (err) {
      return Promise.reject(
        new errors.Request(err.message || 'Request failed')
      );
    });
};
...
RestTransport.prototype.prepareRequest = function (op) {
  ...
  this.jar = this.jar || {};
  if (!this.jar.OSESSIONID) {
    this.applyAuth(config);
  } else {
    config.headers.Cookie = 'OSESSIONID=' + this.jar.OSESSIONID;
  }
...
RestTransport.prototype.handleResponse = function (op, prepared, response) {
  if (response.status === 401) {
    // Retry with auth
    var self = this;
    return axios(this.applyAuth(prepared))
      .then(function (response2) {
        if (response2.status > 399) {
          return Promise.reject(new errors.Request('Authorization Error'));
        } else {
          return deserializer.deserializeDocument(response2.data);
        }
      });
  } else {
    return deserializer.deserializeDocument(response.data);
  }
};