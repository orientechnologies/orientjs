...
function Connection(config) {
  ...
  this.enableRIDBags =
    config.enableRIDBags == null ? true : config.enableRIDBags;
  this.closing = false;
...
Connection.prototype.negotiateConnection = function() {
  return new Promise(
    function(resolve, reject) {
      this.logger.debug(
        "negotiating connection to " + this.host + ":" + this.port
      );
      this.socket.once(
        "connect",
        function() {
 ...
 * @promise {Connection} The connection instance.
 */
Connection.prototype.bindProtocol = function() {
  ...
  return new Promise(
    function(resolve, reject) {
      ...
      this.socket.once(
        ...
        function(err) {
          this.logger.debug(
            "connection closed during protocol negotiation: " + err
          );
          if (this.socket) {
      ...
      this.socket.once(
        ...
        function(err) {
          this.logger.debug(
            "connection ended during protocol negotiation: " + err
          );
          if (this.socket) {
            this.socket.removeAllListeners(); // close earlier could have destroyed & set socket to null.
          }
...
Connection.prototype.process = function(buffer, offset) {
  ...
  if (this.queue.length === 0) {
    if (!this.Operation) {
      this.Operation = require("./protocol" +
        this.protocol.constants.PROTOCOL_VERSION +
        "/operation");
    }
    op = new this.Operation();
  ...
  while ((item = this.queue.shift())) {
    ...
    if (status === OperationStatus.READING) {
      ...
      if (result.status.error) {
        ...
        err = new errors.Protocol(
          "Unknown Error on operation id " + op.id,
          result
        );
        deferred.reject(err);
        this.cancel(err);
      ...
      deferred.reject(
        new errors.Protocol("Unsupported operation status: " + status)
      );
    }
  }
  return [offset, status];
