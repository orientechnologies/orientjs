...
function parseValue(input, type, offset, fieldName, reader) {
  ...
  switch (type) {
    ...
    case 11:
      ...
      break;
    case 12:
      parsed = parseEmbeddedMap(input, offset, fieldName,reader);
      break;
    case 13:
...
function parseEmbeddedMap(input, offset,fieldName,reader) {
  let parsed = parseVarintNumber(input, offset);
  let baseOffset = offset;
  offset += parsed.read;
  let map = {};
  for (let i = 0; i < parsed.value; i++) {
    ...
    if (type === -1) {
      ...
      tmp = parseValue(input, type, offset, `${fieldName}.${key.value}`,reader);
    }
    offset += tmp.read;
