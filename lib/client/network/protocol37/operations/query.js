...
module.exports = Operation.extend({
  ...
  writer: function() {
    ...
    const isNamed =
      this.data.params && this.data.params.params ? !Array.isArray(this.data.params.params) : true;
  ...
  readCommandResult: function(fieldName, reader) {
    this.readOps.push(function(data) {
      ...
      this.readString("queryId", function(data) {
        this.data.stream.queryId = data.queryId;
      });
  ...
  readResultSet: function() {
    this.readOps.push(() => {
      ...
      this.readPlan("executionPlan", data => {
        ...
        this.data.stream.metadata = data;
        this.readOps.push(() => {
          let current = {};
          ...
          this.readInt("count", data => {
            this.readRecord(
              data.count,
              record => {
                if(!this.data.stream.closed){
                  this.data.stream.push(record);
                }
              },
              () => {
                this.readHasNext();
  ...
  readHasNext() {
    this.readBoolean("hasNext", data => {
      if (data.hasNext && !this.data.stream.closed) {
        let msg = {
          queryId: this.data.stream.queryId,
        ...
        this.data.stream.db
          ...
          .catch(err => {
            this.data.stream.emit("error", err);
          });
      } else {
        this.data.stream.closed = true;
        this.data.stream.push(null);
      }
    });
  },
