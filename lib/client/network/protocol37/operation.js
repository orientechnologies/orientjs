    ...
    if (code === 3) {
      ...
      this.readByte("pushType", (data, fieldName) => {
        ...
        if (data[fieldName] === 81) {
          ...
          this.readOps.push([function (offset, data) {
...
Operation.prototype.parseRecordId = function(buffer, offset, context, fieldName, reader) {
    var ctx = {};
    var tmpOffset= 0;
    tmpOffset += this.parseShort(buffer,offset,ctx,"clusterId");
    tmpOffset += this.parseLong(buffer,offset,ctx,"clusterPosition");
    context[fieldName]= new RecordId({ cluster : ctx.clusterId, position : ctx.clusterPosition});
    return tmpOffset;
};
