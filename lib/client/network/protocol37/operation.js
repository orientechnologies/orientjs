...
Operation.prototype.writeHeader = function(opCode, sessionId, token) {
  this.writeByte(opCode).writeInt(sessionId || -1);
...
Operation.prototype.writeBytes = function(data) {
  this.writeOps.push([
    constants.BYTES_INT + data.length,
    function(buffer, offset) {
...
Operation.prototype.writeVarintString = function(data) {
  ...
  this.writeOps.push([
    encodedLength.length + encoded.length,
    function(buffer, offset) {
  ...
  this.readOps.push([
    ...
    [
      ...
      function(data, fieldName) {
        data[fieldName] = deserializer.deserialize(
          3,
          data[fieldName],
      ...
      this.readByte("pushType", (data, fieldName) => {
        if (data[fieldName] === 81) {
            ...
            if (data[fieldName] === 3) {
              ...
              this.readOps.push([
                function(offset, result) {
                  if (result.status === 1) {
                    return [Operation.LIVE_RESULT, null, null, result, offset];
                  } else {
                    return [
          ...
          this.readArray("hosts", (data, fieldName) => {
            let items = [];
            for (let i = 0; i < data.hostSize; i++) {
              items.push(() => {
                this.readString("address");
              });
            }
            return items;
          });
          this.readOps.push([
            function(offset, data) {
              return [Operation.PUSH_DATA, offset, data.hosts];
            }
          ]);
        }
      });
...
Operation.prototype.canRead = function(type, buffer, offset) {
  ...
  switch (type) {
    ...
    case "RecordId":
      return length >= offset + constants.BYTES_SHORT + constants.BYTES_LONG;
    case "Record":
      return length >= offset + constants.BYTES_SHORT;
    case "Long":
    ...
    case "String":
      if (length < offset + constants.BYTES_INT) {
        ...
        return (
          length >= offset + constants.BYTES_INT + buffer.readInt32BE(offset)
        );
      }
      break;
    default:
 ...
 */
Operation.prototype.parseByte = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseChar = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseBoolean = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseShort = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseInt = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseLong = function(
  buffer,
  offset,
  context,
  fieldName,
  ...
  context[fieldName] = Long.fromBits(
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseBytes = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseString = function(
  buffer,
  offset,
  context,
  fieldName,
...
/**
 * Parse a RecordId from the given buffer at the given offset and
 * insert it into the context under the given field name.
 * @param  {Buffer} buffer     The buffer to read from.
 * @param  {Integer} offset    The offset to start reading from.
 * @param  {Object} context    The context to add the value to.
...
Operation.prototype.parseRecordId = function(
  buffer,
  offset,
  context,
  fieldName,
  ...
  var ctx = {};
  var tmpOffset = 0;
  tmpOffset += this.parseShort(buffer, offset, ctx, "clusterId");
  tmpOffset += this.parseLong(buffer, offset, ctx, "clusterPosition");
  context[fieldName] = new RecordId({
    cluster: ctx.clusterId,
    position: ctx.clusterPosition
  });
  return tmpOffset;
};
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseRecord = function(
  buffer,
  offset,
  context,
  fieldName,
  ...
  this.readShort("classId", function(record, fieldName) {
    if (record[fieldName] === -1) {
      record.value = new errors.Protocol(
        "No class for record, cannot proceed."
      );
      this.stack.pop();
      this.readOps.push(function() {
        if (reader) {
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseElement = function(
  buffer,
  offset,
  context,
  fieldName,
  ...
  this.readByte("elementType", function(data) {
    ...
    if (type === 4) {
      ...
      this.readShort("t", function(data, key) {
        if (data[key] === 0) {
          this.readChar("type")
            .readShort("cluster")
            .readLong("position")
          ...
          if (reader) {
            ...
            context[fieldName].cluster = -1;
            context[fieldName].position = -1;
            reader.call(this, context, fieldName);
          }
        }
        // TODO in case of link?
        // else if (data[key] === -3) {
        // this.readShort("cluster").readLong("position");
        // }
      });
    }
  });
...
Operation.prototype.parseResult = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseCollection = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseArray = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parseError = function(
  buffer,
  offset,
  context,
  fieldName,
 ...
 * @return {Integer}           The number of bytes read.
 */
Operation.prototype.parsePushedData = function(
  buffer,
  offset,
  context,
  fieldName,
