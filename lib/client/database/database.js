...
class ODatabase extends DB {
  ...
  configure(config) {
    ...
    this.storage =
      config.storage === "plocal" || config.storage === "memory" ? config.storage
        : "plocal";
    this.token = null;
    this.useToken = true;
  ...
  runInTransaction(txWork) {
    return new Promise((resolve, reject) => {
      try {
        const promise = Promise.resolve(txWork(this));
        this.begin();
        promise
          .then(result => {
            if (this.tx) {
              return Promise.all([Promise.resolve(result), this.tx().commit()]);
            } else {
              return Promise.all([
