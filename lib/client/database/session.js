...
class SessionManager {
  ...
  acquireSubscribeSession(op, data) {
    if (this.currentSession != null && this.currentSession.sessionId !== -1) {
      return this.currentSession.server.acquireForSubscribe().then((connection) => {
        let session = this.currentSession;
        return {session, connection};
      });
    } else {
  ...
  sendOnSubscribe(db, op, data, onAcquire) {
    return this.withRetrySession(op, data, () => {
      ...
      this.client.logger.debug('Sending subscribe operation ' + op + ' for database ' + this.name);
  ...
  close() {
    return this.withSession("db-close", {}, () => {
