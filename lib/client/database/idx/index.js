      ...
      return this.db
        .command(
          "INSERT INTO index:" +
            this.name +
            ' (key, rid) VALUES ("' +
            this.db.escape(item.key) +
            '", ' +
            this.db.escape(item.rid) +
            ")"
        )
        .all();
    }.bind(this)
  ...
  return this.db
    .command(
      "INSERT INTO index:" +
        ...
        this.db.escape(key) +
        '", ' +
        this.db.escape(value["@rid"] || value) +
        ")"
    )
    .all()
    .return(this);
...
Index.prototype.delete = function(key) {
  return this.db
    .command("DELETE FROM index:" + this.name + " WHERE key = :key", {
      params: {
        key: key
      }
    })
...
Index.prototype.select = function() {
  return this.db.select.apply(this.db, arguments).from("index:" + this.name);
};
...
Index.create = function(config) {
  ...
  var query = 'CREATE INDEX ' + config.name + (config.ifnotexist === true ? " IF NOT EXISTS " : "");
  ...
  // add engine support for index , implementation for #232
  if (config.engine) {
    query += " ENGINE " + utils.escape(config.engine);
  }
...
Index.drop = function(name) {
  return this.command("DROP INDEX " + name)
    .all()
    .bind(this)
    .then(function() {
      return this.index.list(true);
...
Index.get = function(name, refresh) {
  if (!refresh && this.index.cached && this.index.cached.names[name]) {
    ...
    return this.index
      ...
      .then(function() {
        return (
          this.index.cached.names[name] ||
          Promise.reject(new errors.Request("No such index: " + name))
        );
      });
  } else {
