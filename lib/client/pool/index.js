...
class BasePool extends EventEmitter {
  constructor(config, params) {
    ...
    cfg.min = cfg.min || 2;
    cfg.max = cfg.max || 5;
    this.pool = genericPool.createPool(this.createPool(config, params), cfg);
    this.poolErr = null;
    this.pool.on("factoryCreateError", err => {
      this.poolErr = err;
      const clientResourceRequest = this.pool._waitingClientsQueue.dequeue();
  ...
  acquire() {
    return new Promise((resolve, reject) => {
      this.pool
        .acquire()
        .then(res => {
          if (res instanceof errors.OrientDBError) {
            reject(res);
          }
          resolve(res);
        })
  ...
  createError(err) {
    return new errors.Connection(2, "Connection Error");
  }
...
Pool.prototype.acquire = function() {
  if (this.closing) {
    return Promise.reject(
      new Error("The pool is closing and cannot accept request")
    );
  }
...
function _acquire() {
  ...
  if (this.availableDBs.length > 0) {
    ...
    this.logger.debug(
      "Acquired database resource with sessionId: " + db.sessionId
    );
    return db.open();
    ...
    waitingClient.timeout = setTimeout(
      _checkWaiting.bind(this, waitingClient),
      this.timeout
    );
...
function _checkWaiting(waitingClient) {
  ...
  if (idx !== -1) {
    ...
    waitingClient.deferred.reject(
      new Error(
        "No db connections available. Timeout " + this.timeout + " exceeded."
      )
    );
  }
}
