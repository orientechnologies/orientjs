...
------------------------------------------------------------------------------*/
var readRecord = function(buf, offset) {
...
var serializeFieldValue = function(type, value) {
    ...
    if (type === "string") {
        ...
        result += "[";
        for (var i in value) {
            var inner_type = type_of(value[i]);
...
var deserializeDocument = function(serialized, document) {
    ...
    if (class_index != -1 && serialized.indexOf(":") > class_index) {
        document["@class"] = serialized.substr(0, class_index);
...
var split_values_from = function(value) {
    var result = [];
...
var look_for_comma_index = function(serialized) {
    var delimiters = [];
    for (var i = 0; i < serialized.length; i++) {
        ...
        } else if ((current === ")" || current === "]" || current === "}") && delimiters[delimiters.length - 1] !== "\"" && current === opposite_of(delimiters[delimiters.length - 1])) {
            delimiters.pop();
...
------------------------------------------------------------------------------*/
var readCollection = function(buf, offset) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        var record = readRecord(buf, offset);
