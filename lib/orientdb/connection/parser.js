...
var readRecord = function(buf, objOffset) {
    ...
    // verify that buffer contains entire header
    ...
    if (record.class == -2) {
        ...
        // verify that buffer contains entire record header
        if (headerEnd > buf.length) return null;

        contentLength = readInt(buf, (headerEnd) - 4);

        // verify that buffer contains entire record
        if (headerEnd + contentLength > buf.length) return null;
...
var readCollection = function(buf, info) {
    ...
    for (i = limit; i--; ) {
        ...
        record = readRecord(buf, recordOffset);

        if (recordOffset.offset === offset) {
            // buffer did not contain entire record
            break;
        } else {
...
var writeString = function(data) {
    ...
    var buf = new Buffer(4 + length);
    buf.writeInt32BE(length, 0);
    stringBuf.copy(buf, 4, 0, stringBuf.length);
    return buf;
};
