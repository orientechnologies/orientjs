...
var serializeFieldValue = function(type, value) {
    ...
    if (type === "string") {
        ...
        if (value["@type"] === "d") {
            result = result.concat("(", serializeDocument(value, false), ")");
        } else {
            result = result.concat("{", serializeDocument(value, true), "}");
        }
    } else if (type === "array") {
        ...
        for (var i in value) {
            ...
            if (inner_type === "object") {
                result = result.concat("(", serializeDocument(value[i]), ")");
            } else {
...
var canParseRid = function(rid) {
    return typeof rid === "string" && REGEXP_RID.test(rid);
};
