...
var isDate = function(value) {
	var chk	= Date.parse(value);

	return _.isNaN(chk) ? false : true;
}

var isMD5 = function(value) {
	var regex	= new RegExp(/^[0-9a-f]{32}$/i);
...
var serializeFieldValue = function(value) {
	...
	if(isMD5(value)) {
        ...
        for(var g=0; g<value.length; g++) {
            if (_.isObject(value[g])) {
                result = result.concat(serializedObject(value[g]));
            } else {
                result = result.concat(serializeFieldValue(value[g]));
            }
            if(g < value.length - 1) {
                result = result.concat(',');
            }
...
var hashToSQLSets = function(vertex) {
    if(_.isEmpty(vertex)) {
        return {
            sqlsets:'',
            remaining: {}
