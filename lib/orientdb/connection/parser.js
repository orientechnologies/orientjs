...
var parseConfiguration = function(configString) {
    ...
    var read = function(value, offset) {
        if (value === " ") {
            return null;
        }
    ...
    config.schemaRecordId = read(values[index++], 1);
    config.dictionaryRecordId = read(values[index++], 1);
    if (config.version > 0) {
        config.indexMgrRecordId = read(values[index++], 1);
    } else {
