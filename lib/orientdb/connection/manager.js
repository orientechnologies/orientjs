...
Manager.prototype.addSocketListeners = function(socket, resolve, reject) {
    ...
    var first		= true;
    var connected	= false;
	console.log('manager::addSocketListeners');

    socket.on('connect', function() {
		console.log('manager::socket:connect');
        debug.log('Connected');
        connected = true;
		resolve();
    });
    ...
    socket.on('data', function(data) {
		console.log('manager::socket:data');
        if (first) {
...
Manager.prototype.readResponse = function(data) {
	console.log('manager::readResponse');
	if(data.length === 0) {
        return;
    }
...
Manager.prototype.writeRequest = function(sessionId, operationType, data) {
    var self	= this;
console.log('manager::writeRequest', sessionId, operationType, data);
	return new Promise(function(resolve, reject) {
		data	= data || {};
