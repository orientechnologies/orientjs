    ...
    socket.on("data", function(data) {
        ...
        if (first) {
            ...
            if (self._PROTOCOL_VERSION != version) {
                var errorMessage = "Server protocol version " + version + " is not supported - This driver supports only version " + self._PROTOCOL_VERSION;
...
Manager.prototype.readResponse = function(data) {
    ...
    do {
        ...
        if (!command.started) {
            ...
            if (command.operationType === OperationTypes.COMMAND || command.operationType === OperationTypes.RECORD_LOAD) {
        ...
        bytesRead = command.instance.read((command.operationType !== OperationTypes.COMMAND && command.operationType !== OperationTypes.RECORD_LOAD) ? dataStruct : dataStruct.buffer, function(err, result) {
            if (err) {
                if (command.callback) {
        ...
        if ((command.instance.done && command.instance.done()) || (typeof command.instance.done === "undefined")) {
            command.done = true;
        }
        ...
        } else if (command.instance.done && command.instance.done()) {
