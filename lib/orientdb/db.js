...
Db.prototype.save = function(document, callback) {
  ...
  if (typeof document._id !== "undefined") {
      recordData.clusterId = parseInt(document._id.substr(0, document._id.indexOf(":")));
      recordData.clusterPosition = parseFloat(document._id.substr(document._id.indexOf(":") + 1))
  }
  if (typeof document._ver !== "undefined") {
      recordData.version = document._ver;
  }

  delete document._ver;
  delete document._id;
  ...
  var clusterId = this.getClusterIdByClass(document["_class"]);
  if (clusterId !== -1) {
      ...
      self.addDataCluster(clusterOptions, function(err, clusterId) {
          self.command("create class " + document["_class"] + " cluster " + clusterId, function(err, result) {
