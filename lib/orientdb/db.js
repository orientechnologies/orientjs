...
Db.prototype.getDataSegmentById = function(dataSegmentId) {
    ...
    for (var i = 0; i < this.configuration.dataSegments.length; i++) {
...
function toRidsFromORIDsOfDocument(self, document, callback) {
    ...
    var loadedTuples = 0;
    for (var idx = 0; idx < oRIDsTuples.length; idx++) {
...
function processResult(content, subcontents) {
    ...
    switch (content.type) {
        ...
        case "d":
            ...
            if (subcontents) {
                var subRecords = [];
                for (var idx = 0; idx < subcontents.length; idx++) {
...
function recursivelyReplaceSubRecordsInRecord(currentRecord, subRecords, accumulatedRIDs) {
    ...
    for (var idx = 0; idx < ridsInRecord.length; idx++) {
