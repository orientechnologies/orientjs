    ...
    switch (results.status) {
        ...
        case 108: // 'l'
            for (var idx in content) {
                records.push(processResult(content[idx]));
            }
            break;
...
function processResult(result) {
    ...
    switch (result.type) {
        ...
        case "d":
            if (parser.isNullOrUndefined(result.content)) {
                record = {};
            } else {
                record = parser.deserializeDocument(result.content.toString());
            }
            record["@type"] = "d";
            if (!parser.isNullOrUndefined(result.version)) {
                record["@version"] = result.version;
            }
            record["@rid"] = result.rid;

            if (result.subcontents) {
                var subRecords = [];
                for (var idx in result.subcontents) {
                    var subRecord = processResult(result.subcontents[idx]);
