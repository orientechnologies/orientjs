...
Db.prototype.open = function (callback) {
    ...
    var openCallback = function(err, result) {
        ...
        self.loadRecord(configDoc, function(err, result) {
            
            if (err) { return callback(err); }

            self.configuration = parser.parseConfiguration(result.content.toString());

            self.loadDocument(self.configuration.schemaRecordId, function(err, result) {
                ...
                callback(err, self.server.sessionId);
            });
        });
    };
...
Db.prototype.loadDocument = function(rid, callback) {
    ...
    this.loadRecord(data, function(err, result) {

        if (result.type !== "d") {
            return callback("Expected document, received " + result.type, result);
        }
        ...
        document = parser.deserializeDocument(result.content.toString(), document);
