...
console.log(content.toString())
	this.server.send(OperationTypes.REQUEST_RECORD_CREATE, data, function(error, results){
		...
		record['@rid']		= parser.parseRid('#'+clusterId+':'+results.position);
...
Db.prototype.recordUpdate = function(record, callback) {
	...
	function onUpdate(error, results) {
		...
		self.server.send(OperationTypes.REQUEST_RECORD_UPDATE, data, function(error, results){
			...
			record['@version']	= results.version;
			record['@class']	= self.getClusterById(record['@rid'].clusterId).name;
...
Db.prototype.recordMetadata = function(record, callback) {
	...
	self.server.send(OperationTypes.REQUEST_RECORD_METADATA, data, function(error, results) {
		...
		var record	= {};
		record['@rid']		= parser.parseRid(results.rid);
		record['@type']		= 'd';
		record['@version']	= results.version;
		var clazz			= self.getClusterById(record['@rid'].clusterId).name;
...
Db.prototype._saveRecord = function(document, transaction, callback) {
    ...
    var saveDocument = function(recordData) {
        if (transaction) {
            ...
            if (docsIndex === -1) {
                var action;
                if (parser.isNullOrUndefined(document["@rid"])) {
                    document["@rid"] = parser.toRid(self.getClusterByName(document["@class"]).id, --transaction.clusterPosition);
                    action = "create";
                } else {
                    action = "update";
                }
    ...
    var cluster = this.getClusterByName(document["@class"]);
