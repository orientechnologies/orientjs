...
Db.prototype.loadRecord = function(rid, options, callback) {
    ...
    self.server.send(OperationTypes.RECORD_LOAD, data, function(err, result) {
        ...
        function recursiveFindORIDs(document, accumulator) {
            for (var field in document) {
                ...
                if (parser.isObject(fieldValue) && !parser.isNullOrUndefined(fieldValue["@class"])) {
                    if (fieldValue["@class"] === "ORIDs") {
                        accumulator.push([document, field, fieldValue]);
                    } else {
                        recursiveFindORIDs(fieldValue, accumulator);
                    }
                } else if (parser.isArray(fieldValue)) {
                    for (var idx in fieldValue) {
                        recursiveFindORIDs(fieldValue[idx], accumulator);
                    }
                }
            }
        }

        // array of arrays whose structures is [parentDocument, fieldName, ORIDs to replace]
        var oRIDsInDocument = [];
        recursiveFindORIDs(record, oRIDsInDocument);

        if (oRIDsInDocument.length === 0) {
            return callback(null, record);
        }

        function loadRIDsFromORIDs(oRID, rid, size, accumulatedRIDs, callback) {
            self.loadRecord(rid, function(err, doc) {
                if (err) { return callback(err); }

                var treeNode = parser.deserializeORIDs(doc, accumulatedRIDs);
                size = size || treeNode.treeSize;

                if (accumulatedRIDs.length === size) {
                    callback(null, oRID, accumulatedRIDs);
                }

                if (treeNode.leftRid) {
                    loadRIDsFromORIDs(oRID, treeNode.leftRid, size, accumulatedRIDs, callback);
                }
                if (treeNode.rightRid) {
                    loadRIDsFromORIDs(oRID, treeNode.rightRid, size, accumulatedRIDs, callback);
                }
            });
        }

        var loadedORIDs = 0;
        for (var idx in oRIDsInDocument) {
            var oRID = oRIDsInDocument[idx];
            loadRIDsFromORIDs(oRID, oRID[2].root, 0, [], function(err, originalORID, accumulatedRIDs) {
                if (err) { return callback(err); }
                
                originalORID[0][originalORID[1]] = accumulatedRIDs;
                loadedORIDs++;
                if (loadedORIDs === oRIDsInDocument.length) {
...
Db.prototype.cascadingSave = function(masterDocument, callback) {
    ...
    function recursivelyFindObjectsWithClass(parentDocument, parentField, path, pathsAccumulator) {
        ...
        for (var field in document) {
            if (parser.isObject(document[field])) {
                var currentPath;
                if (path !== "") {
