...
Db.prototype.updateRecord = function(recordData, callback) {
    ...
    switch (recordData.type) {
        ...
        case "d":
        case "f":
            recordType = recordData.type.charCodeAt(0);
            break;
        default:
            return callback("Invalid record type. Valid values are: b, d, and f");
...
Db.prototype.save = function(document, callback) {
    ...
    var saveDocument = function(recordData) {
        ...
        if (!_.isNull(self.transaction)) {
            if (parser.isNullOrUndefined(document["@rid"])) {
                document["@rid"] = "#-1:" + --self.transaction.clusterPosition;
            }
            if (self.transaction.docs.indexOf(document) === -1) {
                self.transaction.docs.push(document);
            }
    ...
    if (cluster !== null) {
    ...
    } else if (_.isNull(self.transaction)) {
        ...
        // TODO TEST MISSING
        callback("Can't create class " + document["@class"] + " inside transaction");
    }
};
...
Db.prototype.begin = function(callback) {
    if (!_.isNull(this.transaction)) {
        callback("There is already a transaction open");
        return;
    }
