...
Db.prototype.loadRecord = function(rid, options, callback) {
    ...
    self.server.send(OperationTypes.RECORD_LOAD, data, function(err, result) {
            ...
            for (var field in document) {
                ...
                if (parser.isObject(fieldValue) && !parser.isNullOrUndefined(fieldValue["@class"])) {
                    if (fieldValue["@class"] === "ORIDs") {
                        accumulator.push({ doc: document, field: field, value: fieldValue });
                    } else {
        ...
        var oRIDsTuples = [];
        recursiveFindORIDs(record, oRIDsTuples);

        if (oRIDsTuples.length === 0) {
            return callback(null, record);
        }

        function loadRIDsFromORIDs(tuple, rid, size, accumulatedRIDs, callback) {
            self.loadRecord(rid, function(err, orid) {
                if (err) { return callback(err); }

                var treeNode = parser.deserializeORIDs(orid);
                size = size || treeNode.treeSize;
                parser.pushArray(accumulatedRIDs, treeNode.ridList);

                if (accumulatedRIDs.length === size) {
                    tuple.doc[tuple.field] = accumulatedRIDs;
                    callback();
                }

                if (treeNode.leftRid) {
                    loadRIDsFromORIDs(tuple, treeNode.leftRid, size, accumulatedRIDs, callback);
                }
                if (treeNode.rightRid) {
                    loadRIDsFromORIDs(tuple, treeNode.rightRid, size, accumulatedRIDs, callback);
                }
            });
        }
        ...
        for (var idx in oRIDsTuples) {
            ...
            loadRIDsFromORIDs(tuple, tuple.value.root, 0, [], function(err) {
                if (err) { return callback(err); }
