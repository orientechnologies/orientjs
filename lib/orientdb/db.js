...
var Db = exports.Db = function(config) {
	...
	if (!config.database) {
		throw new Error('OrientDB: Database name was not provided.');
	}
	if (!config.username) {
		throw new Error('OrientDB: sername was not provided.');
	}
	if (!config.password) {
		throw new Error('OrientDB: Password was not provided.');
	}
	if (!config.host) {
		throw new Error('OrientDB: Host was not provided.');
	}
	if (!config.port) {
		throw new Error('OrientDB: Port was not provided.');
	}
...
Db.prototype.create = function(data, callback) {
	...
	if(!data.databaseName) {
		throw new Error('OrientDB: Database name required to create.');
		return;
	}

	this.databaseName	= data.databaseName;
	this.databaseType	= data.databaseType	= data.databaseType || 'document';
	this.storageType	= data.storageType	= data.storageType || 'local';

    this.server.send(OperationTypes.REQUEST_DB_CREATE, data, callback);
};

Db.prototype.drop = Db.prototype.delete = function(name, type, callback) {
    ...
    this.server.send(OperationTypes.REQUEST_DB_DROP, data, callback);
};
...
Db.prototype.open = function(name, type, callback) {
	var self	= this;
	...
	var openCallback = function(error, results) {
		if(error) {
			return callback(error);
		}
	...
	self.server.init(function(error) {
		...
		self.server.send(OperationTypes.REQUEST_DB_OPEN, data, openCallback);
	});
};
...
Db.prototype.size = function(callback) {
    ...
    this.server.send(OperationTypes.REQUEST_DB_SIZE, function(error, results) {
        if(error) {
        	return callback(error);
        }
...
Db.prototype.countRecords = function(callback) {
	...
	this.server.send(OperationTypes.REQUEST_DB_COUNTRECORDS, function(error, results) {
		if(error) {
			return callback(error);
		}
...
Db.prototype.reload = function(callback) {
	...
	var reloadCallback = function(error, results) {
		if(error) {
			return callback(error);
		}
	...
	this.server.send(OperationTypes.REQUEST_DB_RELOAD, reloadCallback);
};

Db.prototype.command = Db.prototype.command = function(command, options, callback) {
    ...
    self.server.send(OperationTypes.COMMAND, data, function(error, results) {
        if(error) {
        	return callback(error);
        }
...
Db.prototype.recordLoad = function(record, callback) {
	...
	if(record['@rid'] && _.isArray(record['@rid'])) {
...
Db.prototype.recordCreate = function(record, callback) {
	...
	if(record['@rid'] && record['@rid'].clusterId) {
		...
		clusterId	= this.getClusterByClass(clazz).id;
	} else {
		return callback();
	}
	...
	this.server.send(OperationTypes.REQUEST_RECORD_CREATE, data, callback);
};

Db.prototype.recordUpdate = function(recordData, callback) {
	callback = callback || EMPTY_FUNCTION;
	...
	var content		= parser.encodeRecordData(record);
	var type		= parser.decodeRecordType('d');
	var version		= options.version ? parseInt(version.version) : -1;
	var mode		= options.mode && options.mode ? parseInt(options.mode) : 0;
	...
	this.server.send(OperationTypes.REQUEST_RECORD_UPDATE, data, callback);
};
...
Db.prototype.recordMetadata = function(record, callback) {
	if(OperationTypes.SERVER_PROTOCOL_VERSION < 14) {
		callback(new Error('OrientDB: This functionality is is only available in protocol 14 and above. This protocol is available starting with OrientDB 1.4.'))
	}
	...
	this.server.send(OperationTypes.REQUEST_RECORD_METADATA, data, function(error, results) {
		if(error) {
			return callback(error);
		}
...
/*
Db.prototype._deleteRecord = function(document, transaction, callback) {
    ...
    if (transaction) {
        ...
        if (docsIndex === -1) {
            ...
            callback(new Error('OrientDB: Doc has already been marked for action ' + transaction.actions[docsIndex]));
            return;
        }
        callback();
    } else {
...
Db.prototype._saveRecord = function(document, transaction, callback) {
    if (_.isFunction(transaction)) {
        callback = transaction;
        transaction = null;
    }
    ...
    if (parser.isNullOrUndefined(document["@class"])) {
        return callback(new Error('OrientDB: Document must have a class'));
    }
    ...
    if (!parser.isNullOrUndefined(document["@rid"])) {
        ...
        if (!parser.canParseRid(rid)) {
            return callback(new Error('OrientDB: The RID must be a string having the format: #<cluster id>:<cluster position>'));
        }
    ...
    var saveDocument = function(recordData) {
        if (transaction) {
            ...
            if (docsIndex === -1) {
                ...
                callback(new Error('OrientDB: Doc has already been marked for deletion'));
                return;
            }
            callback(null, document);
        } else if (!parser.isNullOrUndefined(recordData.clusterPosition)) {
            self.recordUpdate(recordData, function(error) {
				if(error) {
					return callback(error);
				}

                self.recordLoad("#" + recordData.clusterId + ":" + recordData.clusterPosition, callback);
            });
        } else {
            self.recordCreate(recordData, function(error, results) {
                if(error) {
					return callback(error);
				}

                self.recordLoad("#" + recordData.clusterId + ":" + results.position, callback);
            });
        }
    };
    ...
    if (cluster !== null) {
        ...
        self.createClass(document["@class"], function(error, clusterId) {
            if(error) {
				return callback(error);
			}
...
/* Data Cluster */
Db.prototype.dataClusterAdd	= function(clusterOptions, callback) {
    callback	= callback || EMPTY_FUNCTION;
    var data	= {};
    ...
    this.server.send(OperationTypes.REQUEST_DATACLUSTER_ADD, data, function(error, results) {
        ...
        callback(null, results.number);
    });
};

Db.prototype.dataClusterDrop = function(clusterNumber, callback) {
    callback = callback || EMPTY_FUNCTION;
...
Db.prototype.dataClusterCount = function(clustersId, callback) {
    callback = callback || EMPTY_FUNCTION;
...
Db.prototype.dataSegmentAdd = function(segmentName, segmentLocation, callback) {
    ...
    self.server.send(OperationTypes.REQUEST_DATASEGMENT_ADD, data, function(error, results) {
        ...
        self.reload(function(error) {
            ...
            callback(null, results.segmentNumber);
        });
    });
};
...
Db.prototype.dataSegmentDrop = function(segmentName, callback) {
    ...
    self.server.send(OperationTypes.REQUEST_DATASEGMENT_DROP, segmentName, function(error, successful) {
        if(error) {
			return callback(error);
		}
...
Db.prototype.cascadingSave = function(masterDocument, transaction, callback) {
    ...
    if (orderedPaths.length === 0) {
        callback(new Error('OrientDB: Document cannot be saved'));
        return;
    }
...
Db.prototype.createClass = function(className, superClassName, callback) {
    ...
    function createClass(className, clusterId, callback) {
        ...
        self.command(createClassCommand, function(error) {
            if(error) {
				return callback(error);
			}
    ...
    if (_.isNull(classCluster)) {
        ...
        self.dataClusterAdd(clusterOptions, function(error, clusterId) {
            if(error) {
				return callback(error);
			}
...
Db.prototype.dropClass = function(className, callback) {
    ...
    self.command("DROP CLASS " + className, function(error) {
        if(error) {
			return callback(error);
		}
...
Db.prototype.getClusterByClass = function(className) {
	...
	for(var idx = 0, length = this.clusters.length; idx<length; idx++) {
		console.log(this.clusters[idx].name, className)
		if(this.clusters[idx].name === className) {
			return this.clusters[idx];
		}
	}
