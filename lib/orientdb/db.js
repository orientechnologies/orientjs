...
Db.prototype.commit = function(transaction, callback) {
    ...
    this.server.send(OperationTypes.TX_COMMIT, transaction, function(err, result) {
        ...
        function lookupTxCreateResult(doc, callback) {
            var rid = doc["@rid"];
            var txResults = result.recordsCreated;
            for (var idx = 0, length = txResults.length; idx < length; idx++) {
                var txResult = txResults[idx];
                if (parser.toRid(txResult.fromClusterId, txResult.fromClusterPosition) === rid) {
                    return callback(null, doc, txResult);
                }
            }
            callback("RID " + rid + " not found in TX recordsCreated");
        }

        function lookupTxUpdateResult(doc, callback) {
            var rid = doc["@rid"];
            var txResults = result.recordsUpdated;
            for (var idx = 0, length = txResults.length; idx < length; idx++) {
                var txResult = txResults[idx];
                if (parser.toRid(txResult.clusterId, txResult.clusterPosition) === rid) {
                    return callback(null, doc, txResult);
                }
            }
            callback("RID " + rid + " not found in TX recordsUpdated");
        }

        for (var idx = 0, length = transaction.actions.length; idx < length; idx++) {
            var action = transaction.actions[idx];
            var doc = transaction.docs[idx];
            if (action === "create") {
                lookupTxCreateResult(doc, function(err, doc, txResult) {
                    if (err) { return callback(err); }

                    doc["@rid"] = parser.toRid(txResult.toClusterId, txResult.toClusterPosition);
                    doc["@version"] = 0;
                });
            } else if (action === "update") {
                lookupTxUpdateResult(doc, function(err, doc, txResult) {
                    if (err) { return callback(err); }

                    doc["@version"] = txResult.version;
                });
            }
        }
