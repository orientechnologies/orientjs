...
GraphDb.prototype.createVertex = function(hash, options, callback) {
    if (parser.isFunction(hash)) {
        callback = hash;
        hash = undefined;
        options = {};
    } else if (parser.isFunction(options)) {
        callback = options;
        options = {};
    }
    ...
    self.command("CREATE VERTEX " + clazz + " CLUSTER " + cluster.name, function(err, results) {
        if (err) { return callback(err); }

        var vertex = results[0];
        parser.mergeHashes(vertex, hash);
...
GraphDb.prototype.createEdge = function(sourceVertexOrRID, destVertexOrRID, hash, options, callback) {
    ...
    self.command("CREATE EDGE " + clazz + " CLUSTER " + cluster.name + " FROM " + sourceRID + " TO " + destRID, function(err, results) {
        if (err) { return callback(err); }
