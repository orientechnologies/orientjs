	...
	return new Promise(function(resolve, reject) {
		...
		var checkClassWithShortName	= function(fieldName, className, classShortName, inOutFieldType) {
			...
			if(parser.isNullOrUndefined(self[fieldName])) {
				self.classCreate(className)
					.then(function(results) {
						self.query('ALTER CLASS ' + className + ' SHORTNAME ' + classShortName, null)
							.then(function(results){
								self.query('CREATE PROPERTY ' + className + '.in ' + inOutFieldType, null)
									.then(function(results){
										self.query('CREATE PROPERTY ' + className + '.out ' + inOutFieldType, null)
											.then(function(results){
		...
		var checkVertexClass = function() {
			checkClassWithShortName('vertexClass', VERTEX_CLASS_NAME, 'V', 'LINKSET')
				.then(function(results){
					self.query('ALTER CLASS ' + VERTEX_CLASS_NAME + ' OVERSIZE 2', null)
						.then(function(results){
							return resolve(results);
						})
						.error(function(error) {
		...
		checkORIDs()
			.then(function(results) {
				checkVertexClass()
					.then(function(results) {
						checkClassWithShortName('edgeClass', EDGE_CLASS_NAME, 'E', 'LINK')
							.then(function(results){
								self.reload()
									.then(function(results){
										resolve(results);
...
GraphDb.prototype.open = function(database) {
	...
	return new Promise(function(resolve, reject) {
		Db.prototype.open.call(self, database)
			.then(function(results) {
				checkForGraphSchema(self)
					.then(function(results) {
						resolve(results);
					})
					.error(function(error) {
