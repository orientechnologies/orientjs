...
Command.prototype.read = function(data, callback) {
    ...
    switch (result.status) {
        ...
        case 97: // 'a'
            ...
            break;
        case 108: // 'l'
            result.content = parser.readCollection(buf, offset);
            break;
        case 110: // 'n'
            // TODO
            throw new Error("Not implemented!")
            break;
        case 114: // 'r'
            var record = parser.readRecord(buf, offset);
            data.index = record._end;
            delete record._end;
            result.content = record;
            break;
    }

    // TODO make the readCollection return 
    //data.index = offset;
    data.index = data.buffer.length;
