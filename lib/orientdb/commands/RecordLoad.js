...
function readPayloadStatus(buf, offset) {
    ...
    switch (status) {
        ...
        case 1:
            this.result.status = status;
            this.step = this.steps.indexOf(readRecordContent);
            break;
        case 2:
            this.step = this.steps.indexOf(readSubRecordIdentifiable);
            break;
        default:
...
function readSubRecordRID(buf, offset) {
    ...
    var subcontent = this.result.subcontents[this.result.subcontents.length - 1];
    subcontent.rid = "#" + parser.readShort(buf, offset) + ":" + parser.readLong(buf, offset + parser.BYTES_SHORT);
