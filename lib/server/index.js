...
Server.prototype.shutdown = function () {
  var config = {username: this.transport.username, password: this.transport.password};
  return this.send('server-shutdown', config)
    .return(true);
};
...
Server.prototype.connect = function () {
  ...
  if (!this.configuration.username || !this.configuration.password) {
    return Promise.reject(new errors.Config('Cannot connect to the server without server credentials'));
  }
  var config = {username: this.configuration.username, password: this.configuration.password};
  if (!this.connected) {
