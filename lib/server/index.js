...
/**
 * Configure the connection pool for the server.
 *
...
Server.prototype.connect = function () {
  ...
  this.connecting = this.pool.send('connect', {
...
Server.prototype.send = function (operation, options) {
  ...
  if (~this.sessionId) {
    ...
    return this.pool.send(operation, options);
  }
  else {
    return this.connect()
    .then(function (server) {
      options.sessionId = options.sessionId != null ? options.sessionId : this.sessionId;
      return server.pool.send(operation, options);
    });
  }
};
