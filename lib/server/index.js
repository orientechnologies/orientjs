 ...
 */
Server.prototype.freeze = function (name) {
  if (!name) {
    return Promise.reject(new errors.Config('Cannot freeze, no database specified.'));
  }
  this.logger.debug('Freeze database ' + name);
  return this.send('db-freeze', name)
  .return(true);
}
...
Server.prototype.release = function (name) {
  if (!name) {
    return Promise.reject(new errors.Config('Cannot release, no database specified.'));
  }
  this.logger.debug('Release database ' + name);
  return this.send('db-release', name)
  .return(true);
