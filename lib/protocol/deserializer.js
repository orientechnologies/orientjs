...
function deserializeDocument (serialized, document, isMap) {
  ...
  if (~classIndex && (indexOfColon > classIndex || !~indexOfColon)) {
    document['@class'] = serialized.substring(0, classIndex);
    serialized = serialized.substring(classIndex + 1);
  }
  ...
  var fieldIndex, field, commaIndex, value;

  while (~(fieldIndex = serialized.indexOf(':'))) {
    field = serialized.substring(0, fieldIndex);
    serialized = serialized.substring(fieldIndex + 1);
    if (field.charAt(0) === "\"" && field.charAt(field.length - 1) === "\"") {
      field = field.substring(1, field.length - 1);
    }
    commaIndex = findCommaIndex(serialized);
    value = serialized.substring(0, commaIndex);
    serialized = serialized.substring(commaIndex + 1);
