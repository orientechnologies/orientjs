...
Operation.prototype.buffer = function () {
  ...
  if (!this.writeOps.length) this.writer();
...
Operation.prototype.parseRecord = function (buffer, offset, context, fieldName, reader) {
  ...
  this.readShort('classId', function (record, fieldName) {
    if (record[fieldName] === -1) {
      ...
      this
      .readByte('type', function (record, fieldName) {
        record[fieldName] = String.fromCharCode(record[fieldName]);
      })
      .readShort('cluster')
      .readLong('position')
      .readInt('version')
      .readString('value', function (record, fieldName) {
        record[fieldName] = deserializer.deserializeDocument(record[fieldName]);
        this.stack.pop();
        this.readOps.push.apply(this.readOps, remainingOps);
      });
    }
